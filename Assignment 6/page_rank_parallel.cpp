...
void pageRankEdge(Graph &g, int max_iters, uintV* verticesArray) {
	uintV n = g.n_;
	long edge_count = 0;
	...
	for (int iter = 0; iter < max_iters; iter++) {
		...
		if (process_rank != 0) {
			MPI_Send(pr_next, n, PAGERANK_MPI_TYPE, 0, 0, MPI_COMM_WORLD);
			MPI_Recv(pr_buffer, verticesArray[process_rank+1] - verticesArray[process_rank], PAGERANK_MPI_TYPE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
		// root
		else {
			for (int process = 1; process < n_processes; process++) {
				MPI_Recv(pr_buffer, n, PAGERANK_MPI_TYPE, process, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
				for (uintV u = 0; u < n; u++)
					pr_next[u] += pr_buffer[u];
			}
			for (int process = 1; process < n_processes; process++)
				MPI_Send(&pr_next[verticesArray[process]], verticesArray[process+1] - verticesArray[process], PAGERANK_MPI_TYPE, process, 0, MPI_COMM_WORLD);
		}
		...
		for (uintV v = verticesArray[process_rank]; v < verticesArray[process_rank+1]; v++) {
			if (process_rank == 0)
				pr_next[v] = PAGE_RANK(pr_next[v - verticesArray[process_rank]]);
			else
				pr_next[v] = PAGE_RANK(pr_buffer[v - verticesArray[process_rank]]);
	...
	if (process_rank != 0) {
		...
		PageRankType sum_buffer = 0;
		global_sum += local_sum;		// adding count of the process 0
		for(int process = 1; process < n_processes; process++) {
			MPI_Recv(&sum_buffer, 1, PAGERANK_MPI_TYPE, process, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			global_sum += sum_buffer;
		}
	}
...
int main(int argc, char *argv[]) {
	...
	switch (strategy) {
		case 1:
			pageRankEdge(g, max_iterations, verticesArray);
			break;
		default:
			break;
	}
