...
void triangleCountParallel(Graph &g, uintV start_n, uintV subset_n, uint thread_id){
	...
	for (uintV u = start_n; u < start_n + subset_n; u++) {
	...
	printf("%d, %ld, %f\n", thread_id, local_triangle_count, time_taken);
}
...
void triangleCountSerial(Graph &g) {
	...
	for(uint th = 0; th < n_workers; th++){
		...
		threads[th] = std::thread(triangleCountParallel, std::ref(g), th * (n/n_workers), n/n_workers + (extra_n+th)/(n_workers), th);
	}
